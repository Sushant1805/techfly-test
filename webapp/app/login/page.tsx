...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    // automatically prepend +91 for API request
    const payloadMobile = mobile.startsWith('+91') ? mobile : `+91${mobile}`;
    
    try {
      const res = await fetch(`${API_BASE_URL}/${selectedInstitute}/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mobile: payloadMobile, password })
      });
  ...
  const verifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/${selectedInstitute}/verify-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mobile: payloadMobile, code: otpFullCode, purpose: 'login' })
      });
      const data = await res.json();
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md">
        <h1 className="text-2xl font-bold text-center mb-6 text-gray-800">
          {!isOtpMode ? 'Sign in to EzzyCoach' : 'Verify OTP'}
        </h1>
        ...
        {!isOtpMode ? (
          ...
          <form onSubmit={verifyOtp} className="space-y-4">
            ...
            <button 
              ...
              className="w-full bg-indigo-600 mt-2 text-white py-2 rounded-lg hover:bg-indigo-700 transition font-medium disabled:opacity-50"
            >
